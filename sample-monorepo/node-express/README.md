# Sample Monorepo — Node / Express variant

This is the practice codebase for the Claude Code 102 labs: a small payments
monorepo with three Express services that talk to each other over HTTP and
...
```bash
cp -R sample-monorepo/node-express ~/labs/refund-monorepo
...
3. Prints your next steps: install dependencies, then start `claude`.
