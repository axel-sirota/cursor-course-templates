...
**1. Context Analysis**
- Read `.cursor/context.md` to determine the **Active Stack** and its **Architecture Shape** (web-service, infra-as-code, library, CLI, ui-only, etc.).
...
- **Web Service** (e.g., python-fastapi, node-express, java-spring, go-gin):
  - **Data Model**: Entities/tables, using the ORM/schema convention documented by the active stack's rules (see `.cursor/rules/`).
