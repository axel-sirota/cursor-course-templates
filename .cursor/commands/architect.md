...
- Claude Code: `.cursor/context.md` — look for `## Active Persona` and `## Active Stack`
- Cursor: `.cursor/context.md` — same fields

If persona is missing → treat as `engineer` (default).

Read `.cursor/rules/*.mdc` (or `.cursor/rules/*.mdc`) for stack standards.
...
- Check: does the plan's tech stack match `.cursor/context.md` / `context.md`?
