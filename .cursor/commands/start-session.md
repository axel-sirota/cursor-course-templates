...
- If the user provides a goal — e.g., "Implement POST /users" (web API), "Write Silver transform for sales pipeline" (Spark), "Add dim_customers dbt model" (dbt), "Write nginx Ansible role" (Ansible) — cross-reference it with the Active Phase in `.cursor/context.md`.
