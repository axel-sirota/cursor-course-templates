...
3.  **Action**:
    - If `stacks/{selection}/rules/` exists, copy `stacks/{selection}/rules/*` to `.cursor/rules/`.
    - If `stacks/{selection}/rules/` does NOT exist (e.g. the `blank` stack), skip the copy and tell the user: "No preset rules for this stack — rules are aspirational until you add your own to `.cursor/rules/`."
...
4.  **Verification**: Read `.cursor/context.md` and confirm the stack is active. Confirm `plan/PHASES.md` exists.
...
2.  **Analysis**: Read the plan and extract:
    ...
    - Testing Strategy
3.  **Generation**:
    - Create a custom content for `.cursor/context.md` based on the plan, including the detected **Architecture Shape**.
...
4.  **Rule Selection**:
    ...
    - Copy those rules to `.cursor/rules/` (if the matched stack has a `rules/` directory; if it does not — e.g. `blank` — skip and tell the user rules are aspirational until they add their own).
...
4.  **Generation**:
    - Create a `.cursor/context.md` reflecting the detected state, including the detected (or "Undetected — treated as blank") **Architecture Shape**.
...
5.  **Rule Selection**:
    ...
    - If no stack was detected, fall back to the `blank` stack: no rules to copy, and tell the user rules are aspirational until they add their own to `.cursor/rules/`.
