...
**1. Context Load**
- Read `@.cursor/context.md` to identify the **Strictness Level** and **Stack**.
- Read `@.cursor/rules/*.mdc` to load the style guide.
- Identify the active stack's manifest file(s), test-file convention, and any dependency-vulnerability tool it declares (from the stack's rules or `.cursor/context.md`) so Sections 2-4 below can be applied to the correct ecosystem instead of assuming Python/Node/Java/Go.
...
**4. Testing Gaps**
- Verify critical paths have corresponding tests, using whatever test location/convention the active stack's rules declare (e.g. `tests/`, co-located `_test.go`/`_test.tf` files, or another convention specified in `.cursor/rules`). If no convention is documented, check the repo's actual layout before assuming `tests/`.
