...
This guide provides instructions for the Cursor LLM on how to manage the Git workflow and development lifecycle for Node.js/Express projects. Use this as a reference when the user requests Git operations.
...
- Bumps version in package.json (`npm version [patch|minor|major] --no-git-tag-version`)
...
### Overview
This guide assumes a standard Node.js/Express/TypeScript project structure with proper Git workflow management.
...
- `make quality` (eslint, prettier --check, tsc --noEmit, jest)
...
├── package.json              # Version management and npm scripts
├── package-lock.json
...
- Version management is handled automatically in package.json (via `npm version`)
...
### Important Notes
- **Single repository** workflow for Node.js/Express projects
