...
- Development tooling configured (golangci-lint, gofmt, go vet)
...
**Deliverables:**
- GORM AutoMigrate or golang-migrate migration for [table]
- GORM model + API DTO structs ([Entity], [Entity]Request, [Entity]Response — flat structs, not
  Pydantic-style inheritance)
- [Entity]Repository with interface-at-point-of-use
- [Entity]Service with business logic
- Real handler implementation (dependency-injecting factory function)
- E2E tests passing (`go test ./...`)
...
- All code quality checks pass (`gofmt -l .`, `golangci-lint run`)
...
- [Entity] GORM model (represents core entity)
...
**Shared Libraries Created:**
- Extended [Entity]Repository with GetByID()
...
- [NewEntity] GORM model + DTOs (layered)
...
### High Risk Phases
- [Phase N]: [Reason — complexity, external dependencies, GORM edge cases, etc.]
...
### Blocking Issues
- Database schema changes require careful migration planning (golang-migrate up/down pairs)
- Authentication must work in Phase 0 before implementing protected endpoints
- Foreign key relationships need parent tables to exist first (Postgres FK constraints)

## Testing Strategy

### Per Phase
- E2E test (httptest) written before implementation
- E2E test must fail initially
- Implementation makes the test pass
- Unit tests added for complex business logic (service layer, mocked repository)

### Integration Testing
After Phase 3: Test cross-entity operations
After Phase 6: Complete CRUD testing
After Phase 9: Full system integration tests (`go test ./... -race -cover`)

## Quality Gates

### Before Completing Each Phase
- [ ] All E2E tests passing (`go test ./...`)
- [ ] `gofmt -l .` empty
- [ ] `golangci-lint run` clean
- [ ] `go vet ./...` clean
- [ ] Doc comments on new exported identifiers
- [ ] Phase summary generated
- [ ] Changes committed

### Before Moving to Production
- [ ] All phases complete
- [ ] Full test suite passing (`go test ./... -race -cover`)
...
4. Restart the phase with a new strategy

### Database Rollback
Each migration should be reversible:
- Keep both `.up.sql` and `.down.sql` files (golang-migrate) or note the AutoMigrate diff
- Test rollback procedures (`migrate down 1`)
...
- [Trade-offs considered — e.g. GORM AutoMigrate vs golang-migrate]
