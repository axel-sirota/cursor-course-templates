# Development Lifecycle Guide (Go + Gin)

## Purpose

This guide provides instructions for the Cursor/Claude Code LLM on how to manage the Git workflow and
development lifecycle for Go/Gin projects. Use this as a reference when the user requests Git
operations.
...
- Runs quality checks (`make quality` — `gofmt`, `go vet`, `golangci-lint`, `go test`)
- Pushes feature branch to remote
...
- Runs a final quality check on develop
...
- Tags the release (`git tag vX.Y.Z`) — Go modules are versioned via git tags, not a manifest file
- Pushes develop with release updates
- Creates PR from develop → main
- Provides a release checklist and next steps
...
### Overview
This guide assumes a standard Go/Gin project structure (`cmd/`, `internal/`, `pkg/`) with proper Git
workflow management.
...
All commands automatically run `make quality`, which chains:
...
├── .github/
│   ├── workflows/ci.yml      # go build / go vet / golangci-lint / go test
│   └── pull_request_template.md
├── Makefile                  # Enhanced with Git workflow + Go quality commands
├── go.mod                    # Module + version pin
├── go.sum
├── .golangci.yml
...
1. **Quality checks fail**: Tell the user to fix issues (`gofmt -w .`, `golangci-lint run --fix` where
   applicable) and try again
2. **Rebase conflicts**: Tell the user to resolve conflicts and continue
3. **Not on correct branch**: Tell the user to switch to the appropriate branch
4. **Uncommitted changes**: Tell the user to commit or stash changes first
...
- Go toolchain (1.23+) and `golangci-lint` installed
...
- **PR descriptions are automatically generated** - no need for the user to edit templates
- **Create concise PR descriptions** with Summary + Key Changes format
- Go module versioning is handled via git tags (`git tag vX.Y.Z`), not a manifest field —
  there is no `pyproject.toml`-style version bump step
...
- Fix issues (`gofmt -w .`, address `golangci-lint` findings) and try again

## Development Workflow

### Important Notes
- **Single repository** workflow for Go/Gin projects
