...
- **If** the active context file (`CLAUDE.md`) has an "Active Phase" line, sync it to mirror the current in-progress phase's name (one-line mirror for human skimming). `plan/PHASES.md` remains the source of truth — `CLAUDE.md`'s field is not parsed by any command and is cosmetic only.
