...
- **Store this as `Architecture Shape`** — this is the same field name and vocabulary that `CLAUDE.md`'s `## Architecture Shape` section and `/setup-stack` use, so detect-stack output stays consistent with the rest of the pipeline.
...
- **Action**: Create context and rules for this project only (not reusable)
  1. Generate `CLAUDE.md` with detected configuration (including `Architecture Shape`)
